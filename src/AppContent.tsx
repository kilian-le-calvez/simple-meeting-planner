...
const MEMBERS = {
  ...
  tennis: ["Kilian", "Lena", "Baptiste", "Nono"],
};

const TITLE = {
  sparta: "Dispo pour réunions - Sparta",
  tennis: "Dispo pour casser des culs - Tennis",
};
...
function AppContent({ password }: AppContentProps) {
  ...
  const refreshData = async () => {
    try {
      const updated = await getData(password);
      setData(updated);
    } catch (err) {
  ...
  const submit = async () => {
    ...
    setSaveLoading(true);
    try {
      await saveData(name, availability, password);
    ...
    <div className="w-screen min-h-screen flex flex-row justify-between items-center p-8">
      ...
      <div className="max-w-xs">
        ...
        <div
          ...
          onClick={async () => {
            const debugInfo = await getServerDataDebug(password);
        ...
        <button
          onClick={async () => {
            const debugInfo = await getServerDataDebug(password); // ton JSON
